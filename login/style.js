(function (window, document) {
    var SliderBar = function (targetDom, options) {
        ...
        if (dataList.length > 0) {
            var html = "<div class='slide-box'><div class='slide-img-block'>" +
                ...
                "<div class='slide-title' id='slideHint'> 按住左边滑块，拖动完成上方拼图</div></div></div>";
    ...
    SliderBar.prototype = {
        ...
        cutImg: function () {
            ...
            // left 
            _this.resultX = Math.abs(Math.floor(Math.random() * (_this.imgWidth - cutWidth * 2 - 4))+ cutWidth);
            console.log(_this.resultX);
            // top
            var cutTop = Math.abs(Math.floor(Math.random() * (_this.imgHeight - cutHeight * 2))+ cutHeight);
            console.log(cutTop);
            // 设置样式
